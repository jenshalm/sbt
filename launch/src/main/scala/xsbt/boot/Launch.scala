...
object Launch
{
	...
	def apply(currentDirectory: File, arguments: List[String]): Option[Int] = {
	    val (isLocate, newArgs) = handleLocateFlag(arguments)
		val (configLocation, newArgs2, state) = Configuration.find(newArgs, currentDirectory)
	...
	def locate(currentDirectory: File, config: LaunchConfiguration): Option[Int] = {
	  locateUri(currentDirectory, config) match {
	    case Some(uri) =>
	      // Print out the URI
	...
	/** Locate the URI of a server with the given launch configuration. */
	def locateUri(currentDirectory: File, config: LaunchConfiguration): Option[URI] =
	  config.serverConfig match {
	  	case Some(_) => Some(ServerLocator.locate(currentDirectory, config))
	    case None => None
	  }
